    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (Tags.Items.DYES.contains(player.getHeldItem(handIn).getItem())) {
            if (state.get(HORIZONTAL_FACING) != dir) {
                ...
                worldIn.setBlockState(pos, RankineLists.RED_ASPHALT_BLOCKS.get((RankineLists.RED_ASPHALT_BLOCKS.indexOf(state.getBlock())+1)%RankineLists.RED_ASPHALT_BLOCKS.size()).getDefaultState().with(SIZE,state.get(SIZE)).with(HORIZONTAL_FACING, dir), 3);
            }
