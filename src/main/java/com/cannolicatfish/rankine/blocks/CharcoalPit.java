    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult p_225533_6_) {
        ...
        if (item == Items.FLINT_AND_STEEL || item == ModItems.SPARK_LIGHTER) {
            ...
            for (BlockPos blockpos : BlockPos.getAllInBoxMutable(pos.add(0,-MAX_HEIGHT,0), pos.add(0, MAX_HEIGHT,0))) {
                if (worldIn.getBlockState(blockpos).getBlock() == ModBlocks.CHARCOAL_PIT) {
            ...
            for (BlockPos blockpos : BlockPos.getAllInBoxMutable(pos.add(0,-MAX_HEIGHT,0), pos.add(0, MAX_HEIGHT,0))) {
                if (worldIn.getBlockState(blockpos).getBlock() == ModBlocks.CHARCOAL_PIT) {
                    worldIn.setBlockState(blockpos, ModBlocks.CHARCOAL_PIT.getDefaultState().with(BlockStateProperties.LIT, Boolean.TRUE), 2);
                }
            }
            return ActionResultType.SUCCESS;
        }
