...
import static com.cannolicatfish.rankine.blocks.ModBlocks.INDUCTION_FURNACE_CONTAINER;
...
public class InductionFurnaceContainer extends Container {
    ...
    public InductionFurnaceContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory, PlayerEntity player, IIntArray furnaceData) {
        ...
        tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
            addSlot(new SlotItemHandler(h, 0, 35, 31));
            addSlot(new SlotItemHandler(h, 1, 55,31));
            addSlot(new SlotItemHandler(h, 2, 75,31));
        ...
        if(slot != null && slot.getHasStack())
        {
            ...
            else if(index != 2 && index != 1 && index != 0)
            {
                ...
                if(!AlloyFurnaceRecipes.getInstance().getAlloyResult(stack1, slot1.getStack()).isEmpty())
                {
                    ...
                    else if(InductionFurnaceTile.isFuel(stack1))
                    {
