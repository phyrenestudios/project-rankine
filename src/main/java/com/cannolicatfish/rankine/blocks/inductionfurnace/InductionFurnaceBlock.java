...
public class InductionFurnaceBlock extends Block{
    ...
    public void animateTick(BlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        if (stateIn.get(BlockStateProperties.LIT)) {
            ...
            double d5 = direction$axis == Direction.Axis.X ? (double)direction.getXOffset() * 0.05D : d4;
            double d6 = rand.nextDouble() * 6.0D / 16.0D;
            double d7 = direction$axis == Direction.Axis.Z ? (double)direction.getZOffset() * 0.05D : d4;
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new InductionFurnaceTile();
    }
