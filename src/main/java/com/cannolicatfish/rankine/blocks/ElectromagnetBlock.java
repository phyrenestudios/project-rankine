        ...
        if (!worldIn.isRemote) {
            ...
            if (flag != worldIn.isBlockPowered(pos)) {
                if (flag) {
                    ...
                    {
                        List<BlockPos> blockPosList = BlockPos.getAllInBox(blockpos,blockpos.add(direction.getDirectionVec().offset(direction, type == 1 ? Config.ALNICO_MAGNET_RANGE.get() : Config.RARE_MAGNET_RANGE.get()))).map(BlockPos::toImmutable).collect(Collectors.toList());
                        ...
                        for (BlockPos b : blockPosList)
                        {
                            ...
                            {
                                if (worldIn.getBlockState(b).getMaterial() == Material.IRON || !Config.ELECTROMAGNET_MATERIAL_REQ.get())
                                {
