...
public interface AlloyToolInterface {
...
*/
    ...
    default float getMaxWearPercent(float modstat, float wmodifier)
    {
        return (modstat - wmodifier)/modstat * 100;
    }

    default float getAlloyAttackDamage(String comp, float dmg, AlloyUtils alloy) {
        return dmg + utils.calcDamage(getElements(comp), getPercents(comp)) + alloy.getAttackDamageBonus();
    }

    default float getAlloyAttackSpeed(String comp, float att, AlloyUtils alloy) {
        return Math.min(att + utils.calcAttackSpeed(getElements(comp), getPercents(comp)) + alloy.getAttackSpeedBonus(), 0);
    }
