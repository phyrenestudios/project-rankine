...
public class AlloyTemplateItemOld extends Item {
    public AlloyTemplateItemOld(Properties properties) {
        super(properties);
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (getTemplate(stack).size() != 0) {
            ...
            for (String s : comp.split("-"))
            {
                ...
                ResourceLocation rs = ResourceLocation.tryCreate(nbt.getString(count));

                Item item = ForgeRegistries.ITEMS.getValue(rs);
                if (item != null) {
                    count++;
    ...
    public static void addTemplate(World worldIn, ItemStack stack, AlloyingRecipe recipe, IInventory inv, DyeItem dye) {
        ...
        stack.getOrCreateTag().putString("AlloyData",!nbt.isEmpty() ? nbt : RankineRecipes.generateAlloyString(inv));
    ...
    public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (getTemplate(stack).size() == 0 && !worldIn.isRemote)
        {
            ...
            if (recipeIn != null) {
                AlloyTemplateItemOld.addTemplate(worldIn,stack, recipeIn, new Inventory(inputs), (DyeItem) Items.WHITE_DYE);
            }
