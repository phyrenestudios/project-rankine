...
public class ItemExpireHandler {
    ...
    public static void onItemPickup(ItemExpireEvent event) {
        if (event.getEntity().isInWater()) {
            RockGeneratorRecipe recipe = event.getEntity().getLevel().getRecipeManager().getRecipeFor(RankineRecipeTypes.ROCK_GENERATOR.get(),new SimpleContainer(event.getEntity().getItem()),event.getEntity().getLevel()).orElse(null);
            if (recipe != null) {
