...
public class RankineMultiOreFeature extends Feature<RankineMultiOreFeatureConfig> {
    ...
    @Override
    public boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, RankineMultiOreFeatureConfig config) {
    ...
    public boolean func_207803_a(IWorld worldIn, Random random, RankineMultiOreFeatureConfig config, double p_207803_4_, double p_207803_6_, double p_207803_8_, double p_207803_10_, double p_207803_12_, double p_207803_14_, int p_207803_16_, int p_207803_17_, int p_207803_18_, int p_207803_19_, int p_207803_20_) {
        int i = 0;
                                        ...
                                        if (!bitset.get(k2)) {
                                            ...
                                            if (config.target.getPredicate().test(worldIn.getBlockState(blockpos$mutableblockpos))) {
                                                List<Block> blockList = Arrays.asList(Blocks.STONE,Blocks.GRANITE,Blocks.DIORITE,Blocks.ANDESITE,Blocks.SANDSTONE,Blocks.RED_SANDSTONE,Blocks.NETHERRACK, Blocks.BLACKSTONE,Blocks.BASALT,Blocks.END_STONE,
                                                        RankineBlocks.GRAY_GRANITE.get(), RankineBlocks.GRANODIORITE.get(), RankineBlocks.HORNBLENDE_ANDESITE.get(), RankineBlocks.THOLEIITIC_BASALT.get(), RankineBlocks.GABBRO.get(), RankineBlocks.ANORTHOSITE.get(),
                                                        RankineBlocks.RHYOLITE.get(), RankineBlocks.LIMESTONE.get(), RankineBlocks.WHITE_MARBLE.get(), RankineBlocks.GNEISS.get(), RankineBlocks.MICA_SCHIST.get(), RankineBlocks.SLATE.get(), RankineBlocks.SHALE.get(), RankineBlocks.IRONSTONE.get(), RankineBlocks.BRECCIA.get(), RankineBlocks.PUMICE.get(), RankineBlocks.SCORIA.get(),
                                                        RankineBlocks.PERIDOTITE.get(), RankineBlocks.KIMBERLITE.get(), RankineBlocks.KOMATIITE.get());
                                                if (blockList.contains(worldIn.getBlockState(blockpos$mutableblockpos).getBlock())){
                                                    worldIn.setBlockState(blockpos$mutableblockpos,
                                                            config.state.with(RankineOreBlock.TYPE,blockList.indexOf(worldIn.getBlockState(blockpos$mutableblockpos).getBlock())), 2);
                                                    ++i;
                                                }
                                            }
                                        }
